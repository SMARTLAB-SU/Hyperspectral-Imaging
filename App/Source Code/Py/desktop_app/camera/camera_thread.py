...
# Try to import Picamera2 for native Raspberry Pi 5 camera support
try:
...
class CameraThread(QThread):
    ...
    def _open_camera(self):
        ...
        # 0. Try native Picamera2 first on Raspberry Pi
        if HAS_PICAMERA2:
            self.status_message.emit("Opening camera using native Picamera2 backend...")
            try:
                self.picam2 = Picamera2()
                config = self.picam2.create_preview_configuration(main={"format": "BGR888", "size": (1280, 720)})
                self.picam2.configure(config)
                self.picam2.start()
                self.cap = "picamera2"
                self.status_message.emit("Native Picamera2 connected successfully.")
                self.use_simulation = False
                return
            except Exception as e:
                self.status_message.emit(f"Native Picamera2 initialization failed: {str(e)}. Trying V4L2/OpenCV fallback...")
            ...
            if self.cap == "picamera2":
                try:
                    ...
                    if raw_frame is None:
                        raise Exception("Captured empty frame array from Picamera2")
                except Exception as e:
                    self.status_message.emit(f"Picamera2 capture failed: {str(e)}. Launching simulation.")
                    self.use_simulation = True
                    self.frame_id += 1
                    raw_frame = self.generate_mock_tissue(self.frame_id)
            else:
